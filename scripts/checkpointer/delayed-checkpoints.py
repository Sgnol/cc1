...
def height():
    base_url = 'http://localhost:11877/getheight'
...
def rpc(method, params={}):
    base_url = 'http://localhost:11877/json_rpc'
    payload = {
        'jsonrpc': '2.0',
        'id': 'block_info',
        'method': method,
    ...
    if 'result' not in resp:
        print ('Unexpected response, make sure CryptoYucd is running with block explorer enabled'
               , resp)
        sys.exit(-1)
    else:
        return resp['result']
